...
HttpPoller.prototype.poll = function() {
  ...
  if (typeof(this.target.body) !== 'string') {
    this.target.body = JSON.stringify(this.target.body);
  }
  this.request.end(this.target.body);
  this.debug("polled: " + this.target.hostname);
  this.target.body && this.debug("body: " + this.target.body);
};
